...
describe("consumeMyNextMessage", () => {
  ...
  it("atomically claims and returns the oldest unread message", () => {
    const dbPath = freshDb();
    seed(dbPath, { team: "team", from_agent: "a1", to_agent: "agent", body: "First", created_at: "2026-01-01T00:00:00Z" });
    seed(dbPath, { team: "team", from_agent: "a2", to_agent: "agent", body: "Second", created_at: "2026-01-01T00:00:01Z" });
    const db = openDb(dbPath);
    const msg = consumeMyNextMessage(db, { dbPath, teamName: "team", agentName: "agent" });
